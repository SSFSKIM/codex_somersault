...
- [x] (2026-07-17 06:05Z) Exit gate pass 9: FIRST run was a codex-side glitch — it resolved the merge base to a SHA that is a "bad object" in the plugin repo and reviewed an empty diff. ROOT CAUSE (found post-hoc): that SHA, d6e60c6c2d…, is actually one of MY OWN commits in the ADJACENT monorepo (codex_somersault) — codex's review agent, running in the plugin repo but with the monorepo sitting next to it on disk, crossed its git context between the two repositories. This is the same root cause as the filesystem-wandering that timed out/killed later passes: codex's review agent does not stay scoped to the target repo. Re-ran and it resolved correctly. Verified my git state was correct (branch 11 commits ahead of main, real 9-file/249-insertion diff intact) and RE-RAN. The real pass-9 found 2 findings [P1], both coverage-loss-guard gaps, ALL fixed: (#1) a failed refuter on a still-PLAUSIBLE P0/P1 finding is real coverage loss (the missing vote could have PROMOTED it to CONFIRMED and flipped the verdict) but my guard treated all refuter failures as harmless → now split by finding status (CONFIRMED failure harmless, PLAUSIBLE failure = coverage loss); (#2) the clean-pass guard's "failed finder lens / L<n>" label didn't cover the SWEEP finder (a SWEEP ASSIGNMENT, not a lens), so a failed sweep at xhigh/max slipped past → the interrupted-review condition now names lens finder, sweep finder, first-pass verifier, and PLAUSIBLE-refuter explicitly.
- [~] Exit gate pass 10: codex became UNRELIABLE for this repo — 3 background kills + 2 foreground 10-min timeouts in a row (its review agent wanders the broader filesystem, bloating each run to ~900KB and >10min; one earlier run also glitched the merge-base to a bad-object temp SHA). Per the doperpowers exit-gate contract ("a fresh Claude reviewer subagent if codex is unavailable"), dispatched an independent opus reviewer carrying the codex rubric against the full 9-file diff, with explicit instructions to check YAML validity and hunt the invariant-violation classes (false-clean-pass paths, verdict determinism, routing gaps, out-of-diff anchors, non-determinism). It traced every load-bearing invariant CLEAN and found only ONE defect — a [P3] cosmetic Synthesis ordered-list numbering gap (1,2,4..10, missing 3, a leftover from the pass-4 step insertion) — now fixed. Verdict: patch is correct, 0.82. A final codex attempt (pass-10d) was left running as a belt-and-suspenders check.
- [x] (2026-07-17 06:40Z) codex-10d (4th pass-10 attempt) killed like the rest → codex confirmed unavailable; accepted the independent Claude-reviewer clean as the sanctioned exit-gate fallback.
- [x] (2026-07-17 06:45Z) Merged `feat/effort-levels` → `main` (--no-ff), tagged v0.2.0, pushed both to origin (github.com/SSFSKIM/codex-review; main f76ab0d, tag live). Plugin validate passes on merged main.
- [x] (2026-07-17 06:50Z) Outcomes & Retrospective written; plan committed to the monorepo.
...
Shipped as codex-review v0.2.0 — merged to `main`, tagged, and pushed to github.com/SSFSKIM/codex-review (main at f76ab0d, tag v0.2.0 live). The plugin now has the scale dial it lacked: `plain` is byte-identical to the v0.1.0 single-reviewer Codex-parity path and remains the default, and `medium`/`high`/`xhigh`/`max` run a lens-partitioned finder → independent verifier → sweep → adversarial-vote DAG that preserves the exact `## Findings`/`## Verdict` output contract at every level. Against the original purpose: a user (or the proactive post-implementation trigger) can now ask for "codex review high against main" and get a multi-agent review, or a bare "codex review" and get exactly today's behavior — the level changes the machinery, never the output shape.

What was validated. The six-fixture test matrix (T1–T6) passed end-to-end with real subagent dispatches: plain reproduced v0.1.0, medium/high found and verified the planted bugs, xhigh's sweep ran, max's two-refuter unanimity vote held, and the clean-control returned `No findings.`/`patch is correct`. The high-level fan-out even surfaced a real *unplanted* bug in the fixture (a first-item-only currency guard) — direct evidence that attention partitioning finds defects a single pass misses.

The exit gate is the story. Codex's native review found and I fixed defects across nine substantive passes (6, 3, 3, 2, 2, 3, 2, 1, 2 findings), then an independent opus reviewer (the sanctioned fallback once codex became unreliable) traced every load-bearing invariant clean and found only a cosmetic list-numbering slip. Roughly 27 real spec defects total — none false positives, and critically, *none caught by my own fixture testing*. The reason is structural and worth remembering: my fixtures verified the happy path of each rule; codex verified the rules' internal consistency. Those are different failure classes. The single most consequential catch (pass 8) was a concrete regression — my pass-5 routing text put a colon-space into the agent's `description:` YAML scalar, which broke the frontmatter parse and silently loaded the default agent with *empty metadata*, disabling the entire plain path since that commit. Neither the fixtures (run before pass 5) nor passes 6–7 caught it; `claude plugin validate` did, once codex pointed at it.

The dominant defect class was coverage-loss laundering: every way a worker failure or a volume cap could quietly turn "we didn't check" into "patch is correct." I closed it incrementally (per-worker-type) and it kept exposing an adjacent worker type until the enumeration was complete — a sign that a prose-expressed orchestration protocol has a large corner-case surface that the plain single-reviewer path simply doesn't have, because there one agent's judgment absorbs all of it. That is the true, non-obvious cost of multi-agentification, and it is now documented in the parity notes.

What remains / follow-ups (none blocking): (1) the deferred Workflow-tool deterministic-DAG variant (a v0.3.0 option, no design change needed); (2) a live-install smoke test after a marketplace refresh; (3) codex's own review-agent unreliability on this repo — root-caused to git-context bleed from the adjacent monorepo (the "bad object" merge-base SHA was literally one of this plan's own monorepo commits, and the filesystem wandering is the same unscoped-agent behavior) — so a future external review of a nested/adjacent repo should run codex from an isolated checkout to keep its agent scoped. Lessons: run `claude plugin validate` after every frontmatter edit (don't outsource that to the reviewer); a cross-model exit gate earns its cost precisely because it catches the class the author's own tests are blind to; and when a "cap" is added to bound cost, prefer batching over dropping — a silent drop is a coverage hole wearing a performance costume, and it took three passes (4→6→7) to fully unwind that one shortcut.
...
- 2026-07-17: Execution complete. Progress tracked through all nine substantive exit-gate passes plus the codex-unreliability fallback; Decision Log verdict-rule entry revised (P0/P1 gate → any-CONFIRMED) per exit-gate finding; Surprises captured the grouping non-transitivity, the recall/precision N+1 refutation, the sweep's unplanted find, the no-execute-hostile-code hardening, the six-then-more spec-contradiction waves, and the YAML-frontmatter regression; Outcomes & Retrospective written at finish. Reason: living-document contract. Shipped v0.2.0.
