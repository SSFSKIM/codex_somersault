...
- [ ] Milestone 4: validation matrix in progress. Done: Test B PASS (2026-07-16 20:12Z, skill path: "Full review comments:" + verbatim blocks); Test E PASS (2026-07-16 21:05Z, v4 via agent after two 529-flaked runs and one routing miss: "No findings." + "patch is correct" + 0.97); Test D PASS (2026-07-16 21:20Z, retry 4 via agent after one 529 flake and two inline-routing misses that drove the two hardening fixes 682a050 + cc17965: "Review comment:" + [P1] + verdict blocks, cosmetic bold/bullet drift accepted); README completed (9850301). Test C PASS (2026-07-16 21:40Z, uncommitted clamp bug caught as [P1] at util.py:2, exact contract format, fixture restored after). Test F PASS (2026-07-16 21:55Z, docstring-only control: "No findings." + "patch is correct" + 0.98, no invented findings). MATRIX COMPLETE 6/6.
- [ ] Milestone 5 (started early, in parallel with M4): gate pass 1 returned one legitimate [P1] — Apache-2.0 §4 non-compliance for the verbatim rubric under MIT-only — fixed in 0e47aea (NOTICE + LICENSES/Apache-2.0.txt + SPDX "MIT AND Apache-2.0" + marketplace description). Gate pass 2 cleared the P1 and returned one [P2] — ref-name shell injection in the backup template (native-verbatim text; same exposure upstream) — fixed in 8e7f1bc (ref-as-data hardening in template + agent fallback + parity-doc deviation 5). Gate pass 3 cleared the injection P2 and returned [P1] Apache-2.0 §4(b) per-file modification notices — fixed in ea3a63b (YAML-comment notices in agent/skill frontmatter — parser-stripped, so zero prompt pollution; shell-comment notice in the script; plugin-load smoke test passed). Gate pass 4 cleared §4(b) and returned [P2] unquoted branch in the Step-2 resolver instruction — fixed in the quoting commit (closes the injection class at all three sites: script argv was always safe, backup template hardened, primary instruction now quoted). Gate pass 5 returned [P2] apostrophe-in-ref defeats bare single-quoting — fixed by mandating the POSIX `'\''` escaping idiom in Step 2, the backup template, and the agent fallback (provably safe for every char git allows). Gate pass 6 returned [P2] one leftover double-quoted ref example in the agent's fallback (`"<branch>@{upstream}"`) — fixed to single-quoted form; full-repo sweep found no other shell-context ref sites (the `("{{title}}")` in the commit template is prompt prose, not shell — native-verbatim, left as is). Gate pass 7 running. Remaining: clean pass, merge, publish.
- [x] (2026-07-16 23:30Z) Milestone 5: exit gate CLEAN on the 8th `codex exec review --base main` pass (No findings / patch is correct; codex ran shellcheck, JSON validation, resolver edge-case fixtures, and end-to-end plugin review flows). Passes 1-7 each returned real findings, all fixed or consciously scoped (license §4(a)/(d) → §4(b) → shell-injection ×4 sites → prompt-injection + PowerShell-scope). Merged build/initial-plugin → main (no-ff), created and pushed public repo https://github.com/SSFSKIM/codex-review. Retrospective written below.
...
- Observation: trigger competition with environment-level skills is real. Test E's phrasing ("Run a code review with these instructions...") was answered by the user's own global `code-review` skill machinery instead of this plugin (output showed that skill's finder/verify vocabulary and no `## Findings`/`## Verdict` blocks), while Tests A/B/D routed to the plugin correctly. Headless `claude -p` loads user-level settings by default; `--setting-sources project` isolates the test to plugin-only machinery.
  Evidence: Test E first-run transcript lacked the mandated blocks but reached the correct substance (util.py clean); rerun isolated via `--setting-sources project`. Implication for real installs: in an environment with a competing generic review skill, generic phrases may route elsewhere; branch/commit/uncommitted phrasings routed correctly even unisolated.
- Observation: Test D's first run hit a server-side 529 Overloaded on subagent dispatch (3 attempts); the headless main agent fell back to an inline review — findings substance correct, but the strict output contract drifted ("Overall correctness: incorrect" missing the "patch is" prefix; "Confidence: high" not a float), confirming that format fidelity lives in the agent, not the main loop. Counted as an infrastructure flake, not a plugin defect; D re-run required through the actual agent.
  Evidence: Test D first-run transcript ("529 Overloaded ... I performed the review myself directly"). Test E v3 (with the strengthened "codex-style review" trigger) hit the same 529s: routing to the plugin protocol verified (dispatch attempted, isolated-context design acknowledged, fallback obeyed the ## Findings / No findings. / ## Verdict contract), agent-path completion still pending API recovery.
- Observation: model-mediated dispatch is nondeterministic without an explicit mandate. Test D retry 2 (API healthy) skipped the subagent entirely — the main agent judged a 2-file commit "small" and reviewed it inline, with no contract format. Native's dispatch is deterministic plumbing (Op::Review always spawns the child); the plugin's model-mediated equivalent needs the invariant stated as an instruction. Fixed by adding an "ALWAYS dispatch — never review inline" mandate to SKILL.md Step 3 (commit 682a050), with the native rationale (the main conversation authored the code; isolation is the mechanism).
  Evidence: Test D retry-2 transcript opens "The commit is small (two files), so here is my direct review" with no dispatch attempt and no ## Findings/## Verdict blocks.
- Observation: the reviewer's severity tag for the same planted bug varied across runs (Test A: [P1]; Test B: [P0]) with identical findings substance. Same-model nondeterminism in severity judgment — native Codex reviews exhibit the same property, so this is parity, not a regression.
  Evidence: Test A and Test B transcripts in Artifacts and Notes.
- Observation: the exit-gate review found a security bug that exists verbatim in native Codex. `BASE_BRANCH_PROMPT_BACKUP` (codex-rs/prompts/src/review_request.rs:20) embeds the branch name inside a double-quoted command substitution; git permits ref names containing `$(...)`, so a hostile ref executes as shell syntax when the model runs the suggested command. Because the plugin ported the template verbatim, reviewing the plugin surfaced the upstream defect. Plugin hardened (8e7f1bc); the upstream exposure is a candidate fork patch / upstream report.
  Evidence: codex review [P2] "Escape branch names before constructing fallback commands", with a demonstrated `$(sh<p)`-style ref in its verification run.
...
- Decision: reviewer agent toolset is `["Read", "Grep", "Glob", "Bash"]`, `model: inherit`.
  ...
  Date/Author: 2026-07-16, plan author.
- Decision: rubric-derived content stays Apache-2.0 with full compliance packaging (NOTICE file carrying the upstream "OpenAI Codex / Copyright 2025 OpenAI" attribution, vendored LICENSES/Apache-2.0.txt, SPDX "MIT AND Apache-2.0" in plugin.json), while the plugin's original content stays MIT.
  Rationale: the Milestone-5 native codex review flagged a legitimate [P1] — shipping the verbatim rubric under MIT-only violates Apache-2.0 §4(a)/(d), and upstream Codex does carry a NOTICE file, so the attribution duty is live. Rejected: relicensing the whole plugin Apache-2.0 (user precedent prefers MIT for their plugin artifacts; dual-licensing is the standard fix); rewriting the rubric to avoid derivation (destroys the verbatim-parity directive).
  Date/Author: 2026-07-16, plan author (exit-gate driven, commit 0e47aea).
- Decision: SKILL.md Step 3 opens with a hard "ALWAYS dispatch the codex-reviewer agent — never review inline" mandate, and the skill description gained distinctive "codex review" / "codex-style review" trigger phrases.
  Rationale: two live-test failure modes forced both. (1) Generic phrasing ("run a code review") routes to Claude Code's built-in review machinery, which loads regardless of setting sources — distinctive phrases give users a reliable route (parity note: native custom reviews are also explicit invocations, never inferred). (2) With a small diff the main agent otherwise reviews inline at its own discretion, silently dropping the isolation invariant that native enforces structurally. Rejected: renaming the skill to avoid collision (unnecessary; phrases suffice); a hook-based hard gate (overkill, not a plugin-shaped mechanism).
  Date/Author: 2026-07-16, plan author (live-test driven).
...
Achieved: a self-contained Claude Code plugin, published at https://github.com/SSFSKIM/codex-review (MIT + Apache-2.0, v0.1.0), that replicates OpenAI Codex's native `codex review` workflow with minimum deviation. It ships the `codex-reviewer` agent (verbatim rubric port + hybrid findings/verdict contract), the `codex-review` skill (four review targets, verbatim seed-prompt templates, merge-base precomputation), the `resolve_target.sh` merge-base port, and a `codex-parity.md` mechanism map. Validated end-to-end: the 6-target fixture matrix (Tests A-F) all passed — planted bugs caught with correct `[P]` tags and `patch is incorrect` verdicts, a clean control returned `No findings` / `patch is correct` with no invented findings — and the branch is `codex exec review`-clean.

Against the original purpose: the user can now `claude --plugin-dir` (or install from the marketplace) and get, natively in Claude Code, the same isolated-reviewer workflow Codex runs — the main agent dispatches a fresh-context reviewer that investigates the diff with git and hands back a findings + verdict block. Parity was preserved at every layer that carries behavior; every deviation is enumerated in the Decision Log and `codex-parity.md`.

What remains / follow-ups: (1) the marketplace install path (`/plugin marketplace add SSFSKIM/codex-review`) is documented but was not live-tested end-to-end — only `--plugin-dir` loading was. (2) The shell-injection class the reviewer surfaced in the verbatim backup template exists identically in upstream Codex (`codex-rs/prompts/src/review_request.rs:20`, `BASE_BRANCH_PROMPT_BACKUP`) — a candidate fork patch and/or upstream report. (3) Windows/PowerShell resolution is consciously out of scope (POSIX-shell assumption documented).

Lessons learned:
- The isolation invariant does not survive model-mediated dispatch for free. Native enforces "review runs in a separate child session" as deterministic plumbing (`Op::Review` always spawns); in a prompt-driven plugin the main agent will review small diffs inline unless explicitly forbidden. It took two hardening commits (mandate in the skill body, then the same mandate in the agent *description* where the router always sees it) to make dispatch reliable.
- Distinctive trigger phrases matter when a generic skill name collides with a built-in. "run a code review" routed to Claude Code's own review machinery regardless of setting sources; adding "codex review" / "codex-style review" gave users a reliable route.
- Reviewing a verbatim port with the tool it ports found a latent bug in the original — the strongest possible evidence of fidelity, and a reminder that "faithful copy" copies defects too.
- The LLM exit-reviewer has a long tail: eight passes, each surfacing a real-but-progressively-more-marginal issue (blocking license → file-level license → four distinct shell-injection sites → prompt-injection → portability). The discipline that ended it was the ExecPlan's "clean OR consciously rejected" bar plus a written scope boundary, not chasing every finding to zero.
