...
## 0. Verified corrections (errata)

The sections below were drafted from a research pack. Re-reading the **actual** Codex source
surfaced eight points to reconcile. Four confirm the original text; four require code-level
changes. They are folded into the relevant sections below; this table is the index.

| # | Topic | Verdict | What changed |
|---|---|---|---|
| 1 | Doc-sync handle in `ExtensionData` | **CORRECTED** | `ExtensionData::get::<T>()` downcasts an `Arc<dyn Any + Send + Sync>` (`ext/extension-api/src/state.rs`), so `T` must be a concrete `Sized` type. `get::<dyn ApplyPatchDocSync>()` (§10-D) does **not** compile. Store/read a concrete newtype `ApplyPatchDocSyncHandle(Arc<dyn ApplyPatchDocSync>)` instead. |
| 2 | Runtime `Config` location | **CONFIRMED + located** | `mcp_servers` *is* a real field on the runtime `Config` at `codex-rs/core/src/config/mod.rs` (~line 753, `Constrained<HashMap<..>>`). Add `pub lsp_servers: HashMap<String, LspServerConfig>` there (plain `HashMap`, no `Constrained` needed), populated from `ConfigToml`. Extension reads `input.config.lsp_servers`. |
| 3 | Config type: no `schema_with` | **CONFIRMED** | `mcp_servers` needs a custom `schema_with` only because of its raw/validated split. A single `LspServerConfig` with `#[derive(JsonSchema)]` in a `HashMap` gets a correct schema automatically. §4/§6 already say this — keep the single-struct shape. |
| 4 | `ToolName` / `ToolExposure` imports | **CONFIRMED** | `ToolName` **is** re-exported by `codex_extension_api`; `ToolExposure` is **not** (it lives at `codex_tools::ToolExposure`, `tools/src/tool_executor.rs`). The direct `codex-tools` dep in §5 stands. |
| 5 | `LspTool::spec()` shape | **CORRECTED** | The real builder is `ToolSpec::Namespace(ResponsesApiNamespace { tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool { parameters: parse_tool_input_schema(&input_schema_for::<LspToolInput>())?, .. })] })`. Copy the `input_schema_for` / `parse_args` helpers from `ext/memories/src/{schema.rs,tools/mod.rs}`. |
| 6 | `AppliedPatchFileChange` variants | **CORRECTED (fields)** | Real variants (`apply-patch/src/lib.rs`): `Add { content, overwritten_content }`, `Update { new_content, old_content, move_path, overwritten_move_content }`, `Delete { content }`. Use `content` (Add), `new_content` (Update); ignore the rest. Map Add→open, Update→change+save, Delete→close. |
| 7 | `ToolCtx` thread-store path | **CONFIRMED** | `ToolCtx` (`core/src/tools/sandboxing.rs`) has `session: Arc<Session>`; the thread store is `ctx.session.services.thread_extension_data` (`core/src/state/service.rs:68`). §5/§10-D path is correct. |
| 8 | `SessionSource::Exec` gate | **CONFIRMED** | `SessionSource` (incl. `Exec`) lives at `codex_protocol::protocol::SessionSource`; `ThreadStartInput.session_source: &SessionSource`. Gate manager creation to skip non-interactive sources as written. |

**Local toolchain note:** `rust-analyzer`, `pyright`, `cargo-nextest`, `just` are present; **Bazel
is absent** (author `BUILD.bazel` + update `Cargo.toml`, but defer `just bazel-lock-update`/
`bazel-lock-check` to CI); `typescript-language-server`/`gopls` are not installed.
...
impl ToolExecutor<ToolCall> for LspTool {
    fn tool_name(&self) -> ToolName { ToolName::namespaced("lsp", "query") }   // ToolName re-exported by codex_extension_api
    // ERRATUM (#5): real builder shape — wrap a Function tool in a Namespace, with the input schema
    // generated by schemars. Reuse the `input_schema_for`/`parse_tool_input_schema` helpers from
    // ext/memories/src/{schema.rs,tools/mod.rs}.
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(ResponsesApiNamespace {
            name: "lsp".to_string(),
            description: default_namespace_description("lsp"),
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                name: "query".to_string(),
                description: /* inline description of the 9 ops */ "...".to_string(),
                strict: false,
                defer_loading: None,
...
| `codex-rs/config/src/config_toml.rs` | `250` (after `mcp_servers`) | Add field: `#[serde(default)] pub lsp_servers: HashMap<String, LspServerConfig>,` with a doc comment. (Unlike `mcp_servers`, no custom `schema_with` is needed because `LspServerConfig` derives `JsonSchema` directly.) Then propagate into the runtime `Config` struct (`codex-rs/core/src/config/mod.rs`, ~line 753, alongside `mcp_servers`) as `pub lsp_servers: HashMap<String, LspServerConfig>` — a plain `HashMap` (no `Constrained` wrapper) — so `LspExtension::on_thread_start` can read `input.config.lsp_servers`. |
...
- Recommendation: **(1a).** Define `ApplyPatchDocSync` in `codex-extension-api` (one additive
  ...
  //
  // ERRATUM (#1): ExtensionData::get::<T> downcasts an Arc<dyn Any + Send + Sync>, so T must be a
  // concrete Sized type. get::<dyn ApplyPatchDocSync>() does NOT compile. We therefore store and
  // read a concrete newtype handle defined in codex-extension-api:
  ...
  pub(crate) fn notify_if_present(thread_store: &ExtensionData, changes: &[AppliedPatchChange]) {
      if let Some(handle) = thread_store.get::<ApplyPatchDocSyncHandle>() {
          handle.0.notify_patch_changes(&map_to_file_changes(changes));
      }
  }
  ```
  The extension stores it at `on_thread_start` via
  `thread_store.insert(ApplyPatchDocSyncHandle(lsp_manager.clone()))` (same concrete-type pattern as
