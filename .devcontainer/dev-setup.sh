...
#   - global Claude Code guidelines installed to ~/.claude/CLAUDE.md (from
#     .devcontainer/claude-global.md) so cloud sessions match local behavior
#
...
echo "==> [5/6] Tuning: env (jobs/stack), mold linker, global CLAUDE.md, optional swap"
...
# Install the user's global Claude Code guidelines so cloud sessions match local.
# Locally ~/CLAUDE.md is loaded as an ancestor of every project; in the cloud we
# place the same content at ~/.claude/CLAUDE.md, which Claude Code reads globally.
GLOBAL_CLAUDE_SRC="$REPO_ROOT/.devcontainer/claude-global.md"
if [ -f "$GLOBAL_CLAUDE_SRC" ]; then
  mkdir -p "$HOME/.claude"
  cp "$GLOBAL_CLAUDE_SRC" "$HOME/.claude/CLAUDE.md"
  echo "    installed global guidelines -> $HOME/.claude/CLAUDE.md"
fi
