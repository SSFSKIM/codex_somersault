...
export function handleIngressMessage(
  ...
  try {
    const parsed: any = normalizeControlMessageKeys(jsonParse(data))
    ...
    if (isSDKControlResponse(parsed)) {
      ...
      onPermissionResponse?.(parsed as SDKControlResponse)
      return
    }
    ...
    if (isSDKControlRequest(parsed)) {
      ...
      onControlRequest?.(parsed as SDKControlRequest)
      return
    }
    ...
    if (parsed.type === 'user') {
      ...
      void onInboundMessage?.(parsed as SDKMessage)
    } else {
      logForDebugging(
