...
export function initializeAnalyticsSink(): void {
  // Somersault: install a no-op sink instead of the real Datadog/1P sink.
  // Drains the queued events with zero I/O. All 262 logEvent call sites
  // continue to work — they just dispatch into the void. Defense in depth
  // on top of privacyLevel.ts essential-traffic hard-default.
  //
  // logEventImpl / logEventAsyncImpl are preserved above. To restore
  // upstream behavior, swap the lambdas below back to the named impls.
  attachAnalyticsSink({
    logEvent: () => {},
    logEventAsync: async () => {},
  })
